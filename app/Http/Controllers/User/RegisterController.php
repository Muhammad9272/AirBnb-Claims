...
$request->validate([
    'name' => [
        ...
        'max:30',
        'unique:users,name',
        'regex:/^[a-zA-Z0-9_\-\. ]+$/', // Added space to allowed characters
        ...
        function ($attribute, $value, $fail) {
            ...
            // Check for excessive spaces (no more than one space between words)
            if (preg_match('/\s{2,}/', $value)) {
                $fail('The username cannot have multiple consecutive spaces.');
            }
            
            // Check if it starts or ends with space
            if (trim($value) !== $value) {
                $fail('The username cannot start or end with spaces.');
            }
            
            // Check for excessive special characters (excluding spaces)
            $specialCharCount = preg_match_all('/[^a-zA-Z0-9 ]/', $value);
            if ($specialCharCount > 2) {
                $fail('The username contains too many special characters.');
            }
            
            // Prevent consecutive special characters (excluding spaces)
    ...
    'name.regex' => 'Username can only contain letters, numbers, spaces, underscores, hyphens, and dots.',
